...
from ..helpers import titlecase as _titlecase
...
@register.filter
def get_pagination_range(value):
    """
    As with get_range with pagination + 1
    """
    return [v + 1 for v in range(value)]


@register.filter()
def constance_get_first(value):
    try:
        word_list = []
        holder = value.split('_', 1)[0].capitalize()
...
@register.filter()
def constance_clean_name(value):
    try:
        words = value.split("_")
        word_list = []
        if len(words) > 1:
            ...
            first_value = value.split('_', 1)[0].capitalize()
            word_list.append(first_value)
        return ' '.join(word_list)
    except ValueError:
        return ''
...
@register.filter()
def constance_buttons(value):
    try:
        word_set = set()
        buttons_list = []
        for x in value:
            first_val = x
            next_value = first_val.itervalues().next()
            current_value = ''.join(next_value).split('_', 1)[0].capitalize()
            word_set.add(current_value)
...
@register.tag('get_recent_tweets')
def get_recent_tweets(parser, token):
    """
    {% get_recent_tweets %}
    """
    return GetRecentTweetsObject()


class GetRecentTweetsObject(Node):
    def render(self, context):
        try:
            tweets = '<br /><br />'.join(str(x) for x in get_tweets()).replace("\"", '').strip()
            tag_matches = re.findall(r'#\w*', tweets)
            if tag_matches:
                for tag in tag_matches:
                    str_tag = ''.join(tag).strip()
                    stripped_tag = str_tag.replace("#", '')
                    tag_url = "<a href=\"https://twitter.com/search?q=%23{0}&src=hash\" rel=\"nofollow\" target=\"_blank\">{1}</a>".format(
                        stripped_tag, str_tag)
                    tweets = tweets.replace(tag, tag_url)
            context['get_recent_tweets'] = tweets
            return ''
        except ValueError:
            return None